 ...
 *  Includes an instrument librarian panel that reads all instrument definitions
 *  found in or below a configurable directory and presents them in a browser
 *  for copying to the clipboard and pasting into the orchestra. 
 *  Note that function definitions ("f" statements) are not read or copied.
 *  <H4>
 *  INSTALLATION
 *  </H4>
...
{
    /**
     *  Reference to the JCsound.Csound object, or internal Csound,
     *  used for rendering.
     */
    public Csound csound = null;
    
    /**
     *  Reference to the Csound process, or external Csound,
     *  used for rendering.
     */
    public Process csoundProcess = null;
    
    /**
     *  Controls whether internal (JCsound.Csound)
     *  or external Csound is used for rendering.
     *  If external Csound is used, the Csound application
     *  is exactly the first token of the Csound command property.
     *  On Windows, this should be "winsound"; on other platforms
     *  it might be different, for example "csound".
     */
    public boolean externalCsound = false;
	...
	public CsoundManager()
	{
		...
		removeScoreButton.setActionCommand("Remove score");
		removeScoreButton.setBounds(12,12,132,24);
		removeScoreButton.setFont(new Font("Dialog", Font.PLAIN, 12));
		removeScoreButton.setForeground(new Color(0));
		removeScoreButton.setBackground(new Color(-3355444));
		gbc = new GridBagConstraints();
		gbc.gridx = 0;
		gbc.gridy = 0;
		...
		gbc.ipadx = 17;
		gbc.ipady = -1;
		((GridBagLayout)scorePanel.getLayout()).setConstraints(removeScoreButton, gbc);
		scorePanel.add(removeScoreButton);
		...
		externalCsoundCheckBox = new javax.swing.JCheckBox();
		externalCsoundCheckBox.setText("External Csound");
		externalCsoundCheckBox.setActionCommand("External");
		externalCsoundCheckBox.setBounds(12,264+30,108,24);
		externalCsoundCheckBox.setFont(new Font("Dialog", Font.PLAIN, 12));
		externalCsoundCheckBox.setForeground(new Color(0));
		externalCsoundCheckBox.setBackground(new Color(-3355444));
		commandPanel.add(externalCsoundCheckBox);
		...
		csoundLabel.setBounds(12,324+36,108,24);
		...
		setPlayerButton.setBounds(132,324+36,108,24);
		...
		setHelpButton = new javax.swing.JButton();
		setHelpButton.setText("Set help...");
		setHelpButton.setActionCommand("setHelp");
		setHelpButton.setBounds(12,324,108,24);
		setHelpButton.setFont(new Font("Dialog", Font.PLAIN, 12));
		setHelpButton.setForeground(new Color(0));
		setHelpButton.setBackground(new Color(-3355444));
		commandPanel.add(setHelpButton);
		helpButton = new javax.swing.JButton();
		helpButton.setText("Help...");
		helpButton.setActionCommand("Help");
		helpButton.setBounds(132,324,108,24);
		helpButton.setFont(new Font("Dialog", Font.PLAIN, 12));
		helpButton.setForeground(new Color(0));
		helpButton.setBackground(new Color(-3355444));
		commandPanel.add(helpButton);
		libraryPanel = new JPanel();
		libraryPanel.setLayout(new GridBagLayout());
		libraryPathButton = new JButton("Directory");
		libraryPathButton.setBounds(12, 12, 108, 24);
		gbc = new GridBagConstraints();
		gbc.gridx = 0;
		gbc.gridy = 0;
		gbc.fill = GridBagConstraints.BOTH;
		...
		gbc.ipadx = 12;
		gbc.ipady = -1;
		((GridBagLayout)libraryPanel.getLayout()).setConstraints(libraryPathButton, gbc);
		libraryPathButton.setFont(new Font("Dialog", Font.PLAIN, 12));
		libraryCopyButton = new JButton("Copy");
		libraryCopyButton.setBounds(24 + 108, 12, 108, 24);
		gbc = new GridBagConstraints();
		gbc.gridx = 1;
		gbc.gridy = 0;
		gbc.fill = GridBagConstraints.BOTH;
		...
		gbc.ipadx = 12;
		gbc.ipady = -1;
		((GridBagLayout)libraryPanel.getLayout()).setConstraints(libraryCopyButton, gbc);
		libraryCopyButton.setFont(new Font("Dialog", Font.PLAIN, 12));
		libraryListScrollPane = new JScrollPane();
		libraryListScrollPane.setBounds(12, 48, 108 + 12 + 108, 336);
		gbc = new GridBagConstraints();
		gbc.gridx = 0;
		gbc.gridy = 1;
		gbc.gridwidth = 2;
		...
		gbc.ipadx = 108 + 12 + 108;
		gbc.ipady = 336;
		((GridBagLayout)libraryPanel.getLayout()).setConstraints(libraryListScrollPane, gbc);
		libraryList = new JList();
		libraryListScrollPane.getViewport().setView(libraryList);
		libraryInstrumentScrollPane = new JScrollPane();
		libraryInstrumentTextArea = new JTextArea();
		libraryInstrumentTextArea.setFont(new Font("MonoSpaced", Font.PLAIN, 12));
		libraryInstrumentScrollPane.getViewport().setView(libraryInstrumentTextArea);
		libraryInstrumentScrollPane.setBounds(108 + 12 + 108 + 24, 48, 336, 336);
		gbc = new GridBagConstraints();
		gbc.gridx = 2;
		gbc.gridy = 1;
		gbc.gridwidth = 2;
		...
		gbc.ipadx = 336;
		gbc.ipady = 336;
		((GridBagLayout)libraryPanel.getLayout()).setConstraints(libraryInstrumentScrollPane, gbc);
		libraryPanel.add(libraryPathButton);
		libraryPanel.add(libraryCopyButton);
		libraryPanel.add(libraryListScrollPane);
		libraryPanel.add(libraryInstrumentScrollPane);
		...
		removeScoreButton.addActionListener(lSymAction);
		...
		setHelpButton.addActionListener(lSymAction);
		helpButton.addActionListener(lSymAction);
		externalCsoundCheckBox.addActionListener(lSymAction);
		libraryPathButton.addActionListener(lSymAction);
		libraryCopyButton.addActionListener(lSymAction);
		SymMouse aSymMouse = new SymMouse();
		libraryList.addMouseListener(aSymMouse);
		libraryCopyButton.addActionListener(lSymAction);
		SymItem lSymItem = new SymItem();
		libraryList.addListSelectionListener(lSymItem);
	    removeAll();
	    if(libraryPath != null)
	    {
	        readAll(libraryPath);
	...
	javax.swing.JButton removeScoreButton;
	...
	javax.swing.JButton setHelpButton;
	javax.swing.JButton helpButton;
	javax.swing.JCheckBox externalCsoundCheckBox;
	javax.swing.JPanel libraryPanel;
	javax.swing.JButton libraryPathButton;
	javax.swing.JButton libraryCopyButton;
	javax.swing.JScrollPane libraryListScrollPane;
	javax.swing.JList libraryList;
	javax.swing.JScrollPane libraryInstrumentScrollPane;
	javax.swing.JTextArea libraryInstrumentTextArea;
	...
	class SymAction implements java.awt.event.ActionListener
	{
		public void actionPerformed(java.awt.event.ActionEvent event)
		{
			...
			else if (object == removeScoreButton)
				removeScoreButton_actionPerformed(event);
			...
			else if (object == setHelpButton)
				setHelpButton_actionPerformed(event);
			else if (object == helpButton)
				helpButton_actionPerformed(event);
			else if (object == externalCsoundCheckBox)
				externalCsoundCheckBox_actionPerformed(event);
			else if (object == libraryPathButton)
				libraryPathButton_ActionPerformed(event);
			else if (object == libraryCopyButton)
				libraryCopyButton_ActionPerformed(event);
		}
	}

	class SymItem implements javax.swing.event.ListSelectionListener
	{
		public void valueChanged(ListSelectionEvent event)
		{
			Object object = event.getSource();
			if (object == libraryList)
				libraryList_ItemStateChanged(event);
		}
	}
	...
	class SymMouse extends java.awt.event.MouseAdapter
	{
		public void mouseClicked(java.awt.event.MouseEvent event)
		{
			Object object = event.getSource();
			if (object == libraryList)
				libraryList_MouseClicked(event);
		}
	}

	void externalCsoundCheckBox_actionPerformed(java.awt.event.ActionEvent event)
	{
	    externalCsound = externalCsoundCheckBox.isSelected();
	    saveConfiguration();
	}
	...
	public void updateView()
	{
        ...
        externalCsoundCheckBox.setSelected(externalCsound);
	}

	void removeScoreButton_actionPerformed(java.awt.event.ActionEvent event)
	{
		removeScore();
		updateView();
	}
	...
	public void start()
	{
	    ...
	    if(csound == null || externalCsound)
	    {
	        try
	        {
	            ...
	            csoundProcess = Runtime.getRuntime().exec(commandLine);
	        }
	        catch(IOException e)
	        {
	            e.printStackTrace();
	...
	public void stop()
	{
	    if(csound == null || externalCsound)
	    {
	        if(csoundProcess != null)
	        {
	            csoundProcess.destroy();
	            csoundProcess = null;
	        }
	    }
	    else
	...
	void setHelpButton_actionPerformed(java.awt.event.ActionEvent event)
	{
        helpCommand = JOptionPane.showInputDialog("Enter Csound help command:");
        saveConfiguration();
	}
	...
	void loadConfiguration()
	{
        try
        {
            ...
            BufferedReader bufferedReader = new BufferedReader(fileReader);
            helpCommand = "c:/ie/IExplore c:/mkg/Csound/AXCsound.htm";
            for(;;)
            {
                ...
                else if(property.compareTo("helpCommand") == 0)
                {
                    helpCommand = value;
                }
                else if(property.compareTo("externalCsound") == 0)
                {
                    externalCsound = Boolean.valueOf(value).booleanValue();
                }
                else if(property.compareTo("libraryPath") == 0)
                {
                    libraryPath = value;
                }
            }
	...
	void saveConfiguration()
	{
        try
        {
            ...
            bufferedWriter.write("helpCommand = " + helpCommand + "\n");
            bufferedWriter.write("externalCsound = " + externalCsound + "\n");
            if(libraryPath != null)
            {
                bufferedWriter.write("libraryPath = " + libraryPath + "\n");
            }
    ...
    /**
     *  Returns the number of instrument definitions in the orchestra.
     */
	...
	/**
	 *  Returns the name of the indicated instrument,
	 *  or null if it does not exist in the orchestra.
     *  The index is the position of the instrument in the ochestra,
     *  not the Csound instrument number.
	 *  The name is the first comment on the same line as the instrument number,
	 *  for example "instr 1 ; Instrument One\n".
     */
	    ...
	    try
	    {
    	    ...
    	    for(int i = 0, n = orchestraTextArea.getLineCount(); i < n; i++)
    	    {
    	        ...
    	        if(instrLineText.startsWith("instr"))
    	        {
    	            ...
    	            if(position == index)
    	            {
                        int commentIndex = instrLineText.indexOf(";");
                        if(commentIndex == -1)
                        {
                            return "Instrument " + String.valueOf(position);
                        }
                        String name = instrLineText.substring(commentIndex + 1).trim();
                        if(name == null)
                        {
                            return "Instr " + String.valueOf(position);
                        }
                        return name;
    	            }
    	        }
	...
	/**
	 *  Returns the Csound number of the indicated instrument,
	 *  or -1 if it does not exist in the orchestra.
     *  Obviously, the index is the position of the instrument in the ochestra,
     *  not the Csound instrument number.
     */
    public int getInstrumentNumber(int index)
    {
	    try
	    {
	        int position = -1;
    	    ...
    	    for(int i = 0, n = orchestraTextArea.getLineCount(); i < n; i++)
    	    {
    	        ...
    	        if(instrLineText.startsWith("instr"))
    	        {
    	            ...
    	            if(position == index)
    	            {
    	                ...
    	                StringTokenizer stringTokenizer = new StringTokenizer(instrLineText, " ,;\n\r\t");
    	                String numberToken = stringTokenizer.nextToken();
    	                return Integer.valueOf(numberToken).intValue();
    	            }
    	        }
    	    }
	    }
    ...
    /**
     *  Returns the index of the named instrument,
	    ...
	    try
	    {
    	    ...
    	    for(int i = 0, n = orchestraTextArea.getLineCount(); i < n; i++)
    	    {
    	        ...
    	        if(instrLineText.startsWith("instr"))
    	        {
                    ...
                    String instrumentName = instrLineText.substring(commentIndex + 1).trim();
                    if(instrumentName.compareTo(name) == 0)
                    {
                        return position;
                    }
    	        }
    ...
    /**
     *  Returns the index for the Csound instrument number,
     *  or -1 if the index is not found.
     *  The index is the position of the instrument in the ochestra,
     *  not the Csound instrument number.
     */
    public int getInstrumentIndex(int instrumentNumber)
    {
	    try
	    {
	        int position = -1;
    	    ...
    	    for(int i = 0, n = orchestraTextArea.getLineCount(); i < n; i++)
    	    {
    	        ...
    	        if(instrLineText.startsWith("instr"))
    	        {
    	            ...
    	            StringTokenizer stringTokenizer = new StringTokenizer(instrLineText, " ,;\n\r\t");
    	            String numberToken = stringTokenizer.nextToken();
    	            int number = Integer.valueOf(numberToken).intValue();
	...
	void libraryList_MouseClicked(java.awt.event.MouseEvent event)
	{
		libraryInstrumentTextArea.setText((String) libraryList.getSelectedValue());
		libraryInstrumentTextArea.setCaretPosition(0);
	}

	void libraryCopyButton_ActionPerformed(java.awt.event.ActionEvent event)
	{
		libraryInstrumentTextArea.selectAll();
		libraryInstrumentTextArea.copy();
		libraryInstrumentTextArea.setCaretPosition(0);
	}

	void libraryList_ItemStateChanged(ListSelectionEvent event)
	{
		libraryInstrumentTextArea.setText((String) libraryList.getSelectedValue());			 
		libraryInstrumentTextArea.setCaretPosition(0);
	}
	
    void readInstruments(File orcfile) throws IOException
    {
        FileReader fileReader = new FileReader(orcfile);
        BufferedReader bufferedReader = new BufferedReader(fileReader);
        String line = null;
        String innerLine = null;
        String innerInnerLine = null;
        while((line = bufferedReader.readLine()) != null)
        {
            if(line.trim().startsWith("<CsInstruments>"))
            {
                while((innerLine = bufferedReader.readLine()) != null)
                {
                    if(innerLine.trim().startsWith("instr"))
                    {
                        StringBuffer stringBuffer = new StringBuffer();
                        int index = innerLine.indexOf(";");
                        if(index != -1)
                        {
                            String name = innerLine.substring(index + 1).trim();
                            stringBuffer.append("; ");
                            stringBuffer.append(name);
                            stringBuffer.append("\n");
                        }
                        stringBuffer.append("; ");
                        stringBuffer.append(orcfile.getAbsolutePath());
                        stringBuffer.append("\n");
                        System.out.println(stringBuffer.toString());
                        stringBuffer.append(innerLine);
                        stringBuffer.append("\n");
                        while((innerInnerLine = bufferedReader.readLine()) != null)
                        {
                            stringBuffer.append(innerInnerLine);                    
                            stringBuffer.append("\n");
                            if(innerInnerLine.trim().endsWith("endin"))
                            {
                                library.addElement(stringBuffer.toString());
                                break;
                            }
                        }
                    }
            ...
            else if(line.trim().startsWith("</CsInstruments>"))
            {
                return;
            }
        }
    ...
    void read(File f) throws IOException
    {
        ...
        {
            File file = new File(f, list[i]);
            String filename = file.getAbsolutePath();
            String[] innerList = file.list();
            if(innerList != null)
            {
                read(file);
            }
            else
            {
                if(filename.endsWith(".orc") ||
                   filename.endsWith(".mml") ||
                   filename.endsWith(".csd"))
                {
                    File orcfile = new File(filename);
                    readInstruments(orcfile);
                }
            }
        }
    }
    
	void libraryPathButton_ActionPerformed(java.awt.event.ActionEvent event)
	{
	    ...
	    System.out.println("Reading all instruments from '" + filename + "' down.");
	    saveConfiguration();
		readAll(libraryPath);
	}
